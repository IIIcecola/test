...
class FFmpegHandler:
    ...
    def reencode_video(self, input_file_path: Path) -> Path:
        ...
        注意:
            - 压缩过程中使用的视频编解码器和音频采样率由配置文件中的 `output_codec` 和 `audio_sample_rate` 决定。
              (cfg.output_code默认为H264, audio_sample_rate默认为Hz44100)
            - 输出视频文件将保存在临时目录中。
        """
        ...
        # 构建音频滤镜
        '''对音频流进行实时处理. 
        aresample：核心重采样滤镜；audio_sample_rate：目标采样率（默认44100Hz）；resampler=soxr：使用SoX高质量重采样算法（比默认快30%）；
        precision=28：28位计算精度（减少舍入误差）；osf=s16：输出格式为16位有符号整数（兼容性最佳）；dither_method=triangular：三角波抖动（消除量化噪声）'''
        audio_filter = [f"aresample={audio_sample_rate}:resampler=soxr:precision=28:osf=s16:dither_method=triangular"]
        '''编解码控制：-c:a aac：转码为AAC格式（主流音频编码）；-b:a {audio_sample_rate}：码率=采样率（如44.1kbps）；-strict experimental：允许实验性编码参数（兼容新特性）'''
        ...
        if frame_rate_adjustment_type == FrameRateAdjustment.Normal: # FrameRateAdjustment.Normal默认使用视频源帧率
            video_filter.append(f"fps=fps={framerate}") 
        elif frame_rate_adjustment_type == FrameRateAdjustment.MotionInterpolation: # 否则minterpolate：运动补偿插帧滤镜
            video_filter.append(
    ...
    def encode_video_to_ts(self, input_video_path: Path) -> Path:
        ...
        # -bsf:v h264_mp4toannexb（H.264格式转换）
    ...
    def run_command(self, command: str, progress_total: int = 0):
        ...
        ''' 
        subprocess.Popen​​：Python subprocess模块中​​最核心、最灵活​​的类，用于创建和管理子进程
        command,  # 要执行的命令（字符串或列表）; stdout=subprocess.PIPE,  # 捕获子进程的正常输出​到一个特殊的管道 (PIPE) 中
        stderr=subprocess.STDOUT, # 将错误输出重定向到正常输出（一起捕获）; universal_newlines=True, # 将输出视为文本（字符串）而非字节流
        encoding='utf-8',    # 指定捕获文本的编码为 UTF-8; shell=True, # 通过系统的 shell 执行命令
        '''
        with subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                              universal_newlines=True, encoding='utf-8', shell=True) as process:
            # 更新进度条
            # 使用迭代器逐行读取子进程输出, iter(process.stdout.readline, '')会在遇到空字符串（流结束）时停止                    
            for line in iter(process.stdout.readline, ''):
                each_line = line.strip() # 去除字符串首尾的空白字符
                if any(x in each_line.lower() for x in self.FFMPEG_ERROR_WORD): # 检查是否包含预设的错误关键词，关键字设置位于FFMPEG_ERROR_WORD
                    loguru.logger.error(each_line)
                else:
                    loguru.logger.debug(each_line)
                if progress_total > 0:
                    '''
                    :=(海象运算符)：在条件判断中同时进行赋值操作; re.search()：正则表达式搜索函数，寻找​​第一个​​匹配你提供的​​正则表达式模式​​的位置
                    正则模式 r'frame=\s*(\d+)': frame=：匹配字面文本, \s*：匹配0个或多个空白字符（空格/制表符）,  
                            (\d+)：捕获1个或多个数字（\d表示数字，+表示1次或多次）, 括号 ()表示捕获组，用于提取数字部分
                    匹配结果：match[0] = "frame= 125"（完整匹配）；match[1] = "125"（第一个捕获组），也就是(\d+)
                    '''
            ...
            # 等待子进程完成并读取所有剩余的输出
            stdout, stderr = process.communicate() # 等待命令执行完成，process.communicate()确保获取所有剩余输出
            if process.returncode != 0: # 命令的退出状态码。 0表示成功，非0表示失败。失败时记录错误并抛出异常
    ...
    def _get_ffmpeg_command(self,
                            ...
                            video_filter: list[str] | None = None, # 视频滤镜
                            audio_filter: list[str] | None = None, # 音频滤镜
                            video_codec: str | None = None, # 视频编码格式
                            audio_codec: str | None = None, # 音频编码格式
                            other_command: list[str] | None = None 
                            ) -> str: # 返回一个字符串
        command: str = f'"{self._ffmpeg_path}" -i "{input_video_path}" ' # 基础命令：告诉FFmpeg要处理哪个文件；-i：表示"输入文件"
        has_audio: bool = self._check_audio_stream_with_ffmpeg(input_video_path)
        # 如果没有音频流，添加静音音频流
        '''-f lavfi: 告诉 FFmpeg 我要使用滤镜来生成输入;  -i anullsrc=...: 具体指定使用 anullsrc这个滤镜来生成输入，并且配置这个滤镜;
        channel_layout=stereo: 配置 anullsrc生成双声道（立体声）的音频;  sample_rate=44100:配置 anullsrc生成采样率为 44.1 kHz 的音频 '''
        silence_audio_command = ' -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 -c:a aac -t 10 '
        
        if video_filter:
            command += ' -filter_complex ' # 告诉FFmpeg要使用复杂滤镜
            command += ' '.join(video_filter) # 把多个滤镜命令连接起来

        if audio_filter and has_audio:
            command += ' -af ' # 音频滤镜标志
            command_without_quote = ','.join(audio_filter) # 把多个音频滤镜用逗号连接
