...
class FFmpegVideoEngine(BaseVideoEngine):
    def __init__(self):
        ...
        self._processor_global_var = ProcessorGlobalVar() # 存储全局处理参数

        self._ffmpeg_handler: FFmpegHandler = FFmpegHandler() # 实际执行 FFmpeg 命令的处理器
        self._signal_bus.set_running.connect(self._set_running)

    def process_video(self, input_video_path: Path) -> Path:
        # 输入视频路径，执行 FFmpeg 命令（._ffmpeg_handler.run_command()）并返回输出路径
        ...
        # 获取音频采样率,_get_audio_sample_rate()内部私有方法
        target_audio_sample_rate: int = self._get_audio_sample_rate()
        loguru.logger.info(f'当前音频采样率为:{target_audio_sample_rate}')

        # 将视频信息转换为VideoInfo对象
        output_file_path = get_output_file_path(input_video_path, 'ffmpeg_processed') # 生成输出路径
        ffmpeg_command: str = self._generate_ffmpeg_commands(input_video_path, # 生成FFmpeg命令，内部私有方法
                                                             output_file_path,
                                                             best_width,
                                                             best_height,
        ...
        self._ffmpeg_handler.run_command(ffmpeg_command, total_frames) # 执行FFmpeg命令。调用FFmpegHandler.run_command()
    ...
    def _generate_ffmpeg_commands(self,
        ...
        # 旋转角度。计算旋转角度（根据方向和宽高比）
        rotate_angle: int = 0
        if video_orientation == Orientation.HORIZONTAL: # 水平视频应用旋转的案例：剪裁存在且剪裁后的宽<高；不剪辑，实际宽<高时；
            if crop and crop.w < crop.h:
                rotate_angle = video_rotation.value
            elif not crop and original_width < original_height:
                rotate_angle = video_rotation.value
        elif video_orientation == Orientation.VERTICAL: # 垂直视频应用旋转的案例：剪辑，宽>高；不剪辑，实际宽>高；
            if crop and crop.w > crop.h:
                rotate_angle = video_rotation.value
            elif not crop and original_width > original_height:
                rotate_angle = video_rotation.value
